...
copy).  A cut whose ref is never pasted degrades to a plain delete.
...
by running them through ``MoveDelta`` — compose/transform/invert calls
made inside a move-aware operation join its transaction via a context
variable at any depth, so captures, window renumbering and routed edits
flow between levels.  Routed edits whose destination window lives inside
embeds are composed back in as minimal embed changes nested along the
destination's hop chain, following the embeds if they were themselves
moved.
...
class ComposeState:
    ...
    depth: int = 0  # nested handler child sequences are deeper than the owner
    retry: bool = False
...
_COMPOSE_STATE: ContextVar[ComposeState | None] = ContextVar('move_compose', default=None)
_INVERT_STATE: ContextVar[InvertState | None] = ContextVar('move_invert', default=None)
_TRANSFORM_STATE: ContextVar[TransformState | None] = ContextVar('move_transform', default=None)


def _walk_move_ops(value, skip_inserts=False):
    """Yield op-shaped dicts carrying a cut or paste, however deeply
    nested inside embed-change payloads.  ``skip_inserts`` leaves out
    move halves riding inserted payloads — content that vanishes
    wholesale when the insert is undone."""
    if isinstance(value, list):
        for item in value:
            yield from _walk_move_ops(item, skip_inserts)
        ...
        for key, item in value.items():
            if skip_inserts and key == 'insert':
                continue
            yield from _walk_move_ops(item, skip_inserts)
...
def check(delta):
    ...
    for operation in _walk_move_ops(list(delta.ops)):
        ...
        if isinstance(spec, dict):
            if (not isinstance(spec.get('start'), int) or spec['start'] < 0
                    or not isinstance(spec.get('length'), int)
                    or spec['length'] <= 0):
                raise ValueError('a paste needs a non-negative integer start '
...
def _compose_embed_change(first, second):
    ...
    return {embed_type: _get_handler(embed_type).compose(
        first_data, second_data, True)}


def _apply_embed_change(insert, change):
    ...
    return {embed_type: _get_handler(embed_type).compose(
        insert_data, change_data, False)}


def _invert_embed_change(change, base_insert):
    embed_type, change_data, base_data = get_embed_type_and_data(
        change, base_insert)
    payload = _get_handler(embed_type).invert(change_data, base_data)
    return {embed_type: payload} if payload else None


def _transform_embed_change(applied, other, priority):
    ...
    if embed_type != next(iter(other)):
        return other
    payload = _get_handler(embed_type).transform(
        applied[embed_type], other[embed_type], priority)
    return {embed_type: payload} if payload else None


def _input_length(operation):
    return 0 if op.type(operation) in ('insert', 'paste') else op.length(operation)


def _output_length(operation):
    return 0 if op.type(operation) in ('delete', 'cut') else op.length(operation)


def _consume_cut(spec, self_it, out, shared):
    ...
    def open_part():
        ...
        if current is None:
            parts += 1
            ref = spec['ref']  # the owner can keep part 1's original ref
            if shared.depth:
        ...
        if piece_type == 'insert':
            ...
            if change is not None:
                for ref, path, inner_offset, _inner in _payload_cut_sites(change):
                    ...
                    shared.trash[ref] = {
                        'ref': part['ref'], 'unit': part['length'],
                        'path': list(path), 'offset': inner_offset}
            segments.append((length, 'base', part['ref'], part['length'],
                             piece.get('attributes'), change))
            part['length'] += length
        remaining -= length
    shared.tables[spec['ref']] = segments


def _expand_pathed(operation, segments):
    ...
    for segment in segments:
        ...
        if 0 <= offset < span:
            ...
            if not isinstance(value, list):
                return None
            pieces = []
            child = MoveDelta(copy.deepcopy(value))
            for piece in child[spec['start']:spec['start'] + spec['length']].ops:
                new_op = {'insert': _apply_embed_change(
                    piece['insert'], spec.get('change'))}
                attributes = op.compose(
                    piece.get('attributes'), operation.get('attributes'), False)
                if attributes:
...
def _expand(operation, tables):
    """Replay one paste window over the segments its cut consumed."""
    spec = operation['paste']
    if 'path' in spec:
        return _expand_pathed(operation, tables[spec['ref']])
    ...
    for segment in tables[spec['ref']]:
        ...
        if low < high:
            ...
            if kind == 'base':
                piece_spec = {
                    'ref': segment[2], 'start': segment[3] + offset, 'length': size}
                change = _compose_embed_change(segment[5], own_change)
                if change is not None:
                ...
                if isinstance(insert, str):
                    ...
                    piece = {'insert': _apply_embed_change(insert, own_change)}
                attributes = op.compose(source.get('attributes'), patch, False)
            else:
                inner = segment[2]['paste']
                piece_spec = {**inner, 'start': inner['start'] + offset,
                              'length': size}
                change = _compose_embed_change(inner.get('change'), own_change)
                if change is not None:
                    piece_spec['change'] = change
                piece = {'paste': piece_spec}
...
def _expanded_payload(value, tables, shared):
    ...
    if isinstance(value, list):
        ...
        for index, item in enumerate(value):
            new_item = _expanded_payload(item, tables, shared)
            if new_item is not None:
                if rewritten is None:
                    rewritten = list(value)
                rewritten[index] = new_item
        return rewritten
    if not isinstance(value, dict):
        return None
    result = None
    sequence = value.get('ops')
    if isinstance(sequence, list):
        changed = False
        merged = MoveDelta()
        for child_op in sequence:
            spec = child_op.get('paste') if isinstance(child_op, dict) else None
            if isinstance(spec, dict):
                if isinstance(spec.get('change'), dict):
                    change = _expanded_payload(spec['change'], tables, shared)
                    if change is not None:
                        child_op = {**child_op,
                                    'paste': {**spec, 'change': change}}
                        spec = child_op['paste']
                        changed = True
                ref = spec.get('ref')
                if ref in tables:
                    for piece in _expand(child_op, tables):
                        merged.push(piece)
                    changed = True
                    continue
                if ref in shared.cuts:
                    shared.retry = True
            if isinstance(child_op, dict) and isinstance(
                    child_op.get('insert'), dict):
                payload = _expanded_payload(child_op['insert'], tables, shared)
                if payload is not None:
                    merged.push({**child_op, 'insert': payload})
                    changed = True
                    continue
            if isinstance(child_op, dict) and isinstance(
                    child_op.get('retain'), dict):
                payload = _expanded_payload(child_op['retain'], tables, shared)
                if payload is not None:
                    merged.push({**child_op, 'retain': payload})
                    changed = True
                    continue
...
def _composed_retain(self_op, other_op, length):
    """One retain pairing.  Returns ``(op, sealed)``: a sealed op went
    through an embed handler, whose child compose already expanded any
    nested windows — the assemble pass must not expand it again."""
    if op.type(self_op) == 'paste':
        spec = dict(self_op['paste'])
        sealed = False
        if isinstance(other_op.get('retain'), dict):
            sealed = spec.get('change') is not None  # handler composed
            spec['change'] = _compose_embed_change(
                spec.get('change'), other_op['retain'])
        new_op = {'paste': spec}
        attributes = op.compose(
            self_op.get('attributes'), other_op.get('attributes'), True)
        if attributes:
            new_op['attributes'] = attributes
        return new_op, sealed
    ...
    if isinstance(self_op.get('retain'), (int, float)):
        ...
        new_op[action] = {
            embed_type: handler.compose(self_data, other_data, action == 'retain')}
        sealed = True
    attributes = op.compose(
...
def _transformed_retain(self_op, other_op, length, priority):
        ...
        if embed_type == next(iter(other_data)):
            ...
            if handler:
                transformed_data = {embed_type: handler.transform(
                    self_data[embed_type], other_data[embed_type], priority)}
    new_op = {'retain': transformed_data}
    attributes = op.transform(
        self_op.get('attributes'), other_op.get('attributes'), priority)
    if attributes:
        new_op['attributes'] = attributes
    return new_op
    ...
    for operation in ops:
        if op.type(operation) == 'paste':
            ...
            if 'path' not in spec:  # trash reads are opaque to routing
                windows.setdefault(spec['ref'], []).append(Window(
                    spec['start'], spec['length'], operation.get('attributes'),
                    spec.get('change'), ('root', position)))
            ...
            _collect_child_windows(operation['retain'][embed_type],
                                   ((position, embed_type),), (), windows)
        elif isinstance(operation.get('insert'), dict):
            embed_type = next(iter(operation['insert']))
            _collect_child_windows(operation['insert'][embed_type],
                                   ((position, embed_type),), (), windows)
...
def _collect_change_windows(spec, position, windows, hops=()):
    ...
    if isinstance(change, dict) and change:
        ...
        _collect_child_windows(change[change_type],
                               (*hops, (position, change_type)), (), windows)


def _collect_child_windows(value, hops, path, windows):
    if isinstance(value, list):
        for index, item in enumerate(value):
            _collect_child_windows(item, hops, (*path, index), windows)
    elif isinstance(value, dict):
        sequence = value.get('ops')
        if isinstance(sequence, list):
            unit, embed_type = hops[-1]
            here = (*hops[:-1], (unit, embed_type, (*path, 'ops')))
            prefix = 0
            for child_op in sequence:
                ...
                if isinstance(spec, dict) and isinstance(spec.get('ref'), str):
                    if 'path' not in spec:
                        windows.setdefault(spec['ref'], []).append(Window(
                            spec['start'], spec['length'],
                            child_op.get('attributes'), spec.get('change'),
                            ('child', here, prefix)))
                    ...
                    _collect_child_windows(
                        child_op['retain'][inner_type],
                        (*here, (prefix, inner_type)), (), windows)
                elif isinstance(child_op.get('insert'), dict):
                    inner_type = next(iter(child_op['insert']))
                    _collect_child_windows(
                        child_op['insert'][inner_type],
                        (*here, (prefix, inner_type)), (), windows)
                prefix += _output_length(child_op)
        for key, item in value.items():
            if key != 'ops':
                _collect_child_windows(item, hops, (*path, key), windows)
                ...
                for window in windows.get(ref, ()):
                    if window.start <= offset < window.start + window.length:
                        for candidate in delta.ops:
                            ...
                            if (spec['ref'] == ref
                                    and spec['start'] == window.start
                                    and spec.get('change') is not None):
                                change = spec['change']
...
def _deposit(shared, ref, windows, index, offset, routed,
             local_pastes=frozenset()):
...
def _route_delete(shared, ref, windows, low, high, local_pastes=frozenset()):
    ...
    for index, run_low, run_high in _covered_runs(windows, low, high):
        _deposit(shared, ref, windows, index,
                 run_low - windows[index].start, {'delete': run_high - run_low},
                 local_pastes)


def _payload_has_pastes(payload):
    return any(isinstance(operation.get('paste'), dict)
    ...
    for ref, path, offset, length in _payload_cut_sites(payload):
        ...
        for index, span in enumerate(spans):
            ...
            for low, high, attributes in rescued:
                ...
                if low < high:
                    if low > cursor:
                        _deposit(shared, ref, spans, index,
                                 cursor - window_low, {'delete': low - cursor})
                    trimmed = op.transform(span.attributes, attributes, True)
                    if trimmed:
                        _deposit(shared, ref, spans, index, low - window_low,
                                 {'retain': high - low, 'attributes': trimmed})
                    cursor = high
            if cursor < window_low + window_length:
                _deposit(shared, ref, spans, index, cursor - window_low,
                         {'delete': window_low + window_length - cursor})
...
def _rewrite_payload(value, state, priority):
    """Renumber paste windows inside an embed payload the transform loop
    passed through untouched."""
    if isinstance(value, list):
        for item in value:
            _rewrite_payload(item, state, priority)
    elif isinstance(value, dict):
        sequence = value.get('ops')
        if isinstance(sequence, list):
            rebuilt = []
            ...
            for child_op in sequence:
                ...
                if isinstance(spec, dict) and spec.get('ref') in state:
                    pieces = _renumber(child_op, state, priority)
                ...
                if isinstance(child_op, dict) and isinstance(
                        child_op.get('retain'), dict):
                    _rewrite_payload(child_op['retain'], state, priority)
                if isinstance(child_op, dict) and isinstance(
                        child_op.get('insert'), dict):
                    _rewrite_payload(child_op['insert'], state, priority)
                rebuilt.append(child_op)
            if spliced:  # re-merge neighbors the splice made adjacent
                merged = MoveDelta()
...
def _unit_coordinate(delta, index):
    ...
    for operation in delta.ops:
        ...
        if kind == 'delete' and 0 <= offset < operation['delete']:
            return None
        input_position += _input_length(operation)
...
def _read_spans(edit_ops, start, end):
    ...
    for child_op in edit_ops:
        ...
        input_length = _input_length(child_op)
        output_length = _output_length(child_op)
        if kind == 'retain':
            low = max(input_position, start)
            high = min(input_position + input_length, end)
            if low < high:
                runs.append([output_position + low - input_position,
                             high - low])
...
def _route_into_reads(edit_ops, reads, read_buckets, state, taken,
                      alive_refs=frozenset()):
    ...
    for child_op in edit_ops:
        ...
        if kind == 'insert':
            for read in reads:
                if read['start'] < position < read['start'] + read['length']:
                    ...
                    attributes = op.compose(child_op.get('attributes'),
                                            read.get('attributes'), False)
                    joined.pop('attributes', None)
                    if attributes:
                        joined['attributes'] = attributes
                    read_buckets[read['key']].append(
                        (position - read['start'], joined))
            continue
        if kind == 'paste':
            continue  # its content dies with the cell or was re-cut
        length = _input_length(child_op)
        if kind == 'cut' and child_op['cut']['ref'] in alive_refs:
            ...
            for read in reads:
                ...
                high = min(position + length,
                           read['start'] + read['length'])
                if low >= high:
                    continue
                if low > cursor:
                    mapping['segments'].append(
                        (low - cursor, None, 0, None, None))
                part = _new_part(mapping, inner_ref, taken)
                read_buckets[read['key']].append(
                    (low - read['start'],
                     {'cut': {'ref': part, 'length': high - low}}))
                mapping['segments'].append(
                    (high - low, part, 0, None, None))
                cursor = high
            if cursor < position + length:
                mapping['segments'].append(
                    (position + length - cursor, None, 0, None, None))
        elif kind == 'cut':
            ...
            for read in reads:
                ...
                if low < high:
                    read_buckets[read['key']].append(
                        (low - read['start'], {'delete': high - low}))
        else:
            for read in reads:
                low = max(position, read['start'])
                high = min(position + length,
                ...
                if kind == 'delete':
                    read_buckets[read['key']].append(
                        ...
                        or isinstance(child_op.get('retain'), dict)):
                    routed = ({'retain': child_op['retain']}
                    ...
                    read_buckets[read['key']].append(
                        (low - read['start'], routed))
        position += length
    return rebased


def _routed_entry(routed, state, priority):
    ...
    if isinstance(routed, tuple):
        ...
        _rewrite_payload(rewritten['retain'], state, priority)
        return rewritten
    return routed
...
def _read_from_trash(result, position, path, entries):
    ...
    for offset, routed in entries:
        ...
        trash_map[routed['cut']['ref']] = {
            'ref': trash_ref, 'unit': unit, 'path': list(path),
            'offset': offset}
...
def _collect_captures(ops, base, captures):
    ...
    for operation in ops:
        if not isinstance(operation, dict):
            continue
        if op.type(operation) == 'cut':
            captures[operation['cut']['ref']] = base[
                position:position + operation['cut']['length']].ops
        elif isinstance(operation.get('retain'), dict):
            unit = base[position:position + 1].ops
            snapshot = unit[0].get('insert') if unit else None
            if isinstance(snapshot, dict):
                _collect_payload_captures(operation['retain'], snapshot,
                                          captures)
        position += _input_length(operation)
...
def _collect_payload_captures(change, snapshot, captures):
    ...
    for key, value in change.items():
        snap = snapshot.get(key) if isinstance(snapshot, dict) else None
        if key == 'ops' and isinstance(value, list) and isinstance(snap, list):
            _collect_captures(value, MoveDelta(list(snap)), captures)
        elif isinstance(value, dict):
            _collect_payload_captures(value, snap, captures)
...
def _lower_ops(ops, captures):
    ...
    for operation in ops:
        ...
        if kind == 'cut':
            ...
            _lower_payload(payload, captures)
            new_op = dict(operation)
            new_op['retain'] = payload
            lowered.push(new_op)
        elif isinstance(operation.get('insert'), dict):
            payload = copy.deepcopy(operation['insert'])
            _lower_payload(payload, captures)
            new_op = dict(operation)
            new_op['insert'] = payload
            lowered.push(new_op)
        else:
...
def _lower_payload(value, captures):
    if isinstance(value, dict):
        sequence = value.get('ops')
        if isinstance(sequence, list):
            value['ops'] = _lower_ops(sequence, captures).ops
        for key, item in value.items():
            if key != 'ops':
                _lower_payload(item, captures)
...
def _capture_read(operation, captures):
    ...
    if 'path' in spec:
        content = _navigate_payload(list(content),
                                    [spec.get('unit', 0), *spec['path']])
        if not isinstance(content, list):
            return []
    pieces = []
    window = MoveDelta(copy.deepcopy(list(content)))[
        spec['start']:spec['start'] + spec['length']]
...
def _apply_overlays(result, shared, priority):
    ...
    for hops, entries in grouped.items():
        child_ops, _span = _laid_out(
            entries,
            lambda routed: _routed_entry(routed, shared.state, priority))
    ...
    for hops, child_ops in items:
        ...
        if located is None:  # the destination embed no longer exists
            ...
            result = _read_from_trash(result, hops[0][0],
                                      _flatten_hops(hops), offsets)
            continue
        ...
        position, embed_type, path = hops[0]
        child_ops = _wrap_hops(hops, child_ops)
        if existing is not None:  # rebase through their own edits there
            child_ops = MoveDelta(copy.deepcopy(list(existing)))._transform_transaction(
                MoveDelta(child_ops), False).ops
            if not child_ops:
                continue
...
def _route_cut(spec, other_it, shared, out, priority, local_pastes,
               local_reads=None, read_buckets=None):
    ...
    while offset < length:
        ...
        piece = (other_it.next(piece_length) if other_it.peek() is not None
                 else {'retain': piece_length})
        piece_type = op.type(piece)
        ...
        if piece_type == 'cut':
            ...
            if priority:
                ...
                read_units = sorted({
                    read['unit'] for read
                    in shared.other_reads.get(their_ref, ())})
                ...
                def drop_run(their_low, their_high):
                    ...
                    for unit in read_units:
                        ...
                        if unit > cursor:
                            mapping['segments'].append(
                                (unit - cursor, None, 0, None, None))
                            _route_delete(shared, ref, windows,
                                          low + cursor - run_start, our_unit,
                                          local_pastes)
                            kill_reads(low + cursor - run_start, our_unit)
                        covering = _covered_runs(windows, our_unit,
                                                 our_unit + 1)
                        if covering:
                            part = _new_part(mapping, their_ref,
                                             shared.taken)
                            for index, run_low, _run_high in covering:
                                _deposit(shared, ref, windows, index,
                                         ...
                                         local_pastes)
                            mapping['segments'].append((1, part, 0,
                                                        None, None))
                        else:
                            # both sides dropped the unit and we win:
                            # their read loses it with everything else
                            mapping['segments'].append((1, None, 0,
                                                        None, None))
                        cursor = unit + 1
                    if cursor < their_high:
                        mapping['segments'].append(
                            (their_high - cursor, None, 0, None, None))
                        _route_delete(shared, ref, windows,
                                      low + cursor - run_start,
                                      low + their_high - run_start,
                                      local_pastes)
                        kill_reads(low + cursor - run_start,
                                   low + their_high - run_start)

                position = run_start
                for their_index, cov_low, cov_high in _covered_runs(
                        theirs, run_start, run_start + piece_length):
                    if cov_low > position:
                    ...
                    if their_window.attributes or their_window.change:
                        for index, run_low, run_high in _covered_runs(
                                windows,
                                low + cov_low - run_start,
                                low + cov_high - run_start):
                            attributes = op.transform(
                                windows[index].attributes,
                                theirs[their_index].attributes,
                                priority)
                            change = _transform_embed_change(
                                windows[index].change,
                                theirs[their_index].change,
                                priority)
                            if change is not None:
                            ...
                            _deposit(shared, ref, windows, index,
                                     run_low - windows[index].start, routed,
                                     local_pastes)
                    mapping['segments'].append(
                        (cov_high - cov_low, None, 0, None, None))
                    position = cov_high
                ...
                def lost_run(our_low, our_high):
                    ...
                    mapping['segments'].append(
                        (their_high - their_low, None, 0, None, None))
                ...
                for index, run_low, run_high in _covered_runs(windows, low, high):
                    ...
                    _deposit(shared, ref, windows, index,
                             ...
                             local_pastes)
                    mapping['segments'].append(
                        (run_high - run_low, part_ref, 0, None, None))
                    position = run_high
                if position < high:
                ...
                if local_reads:
                    ...
                    for read in local_reads.get(ref, ()):
                        ...
                        if _covered_runs(
                                shared.other_windows.get(their_ref, []),
                                their_unit, their_unit + 1):
                            continue
                        read_buckets[read['key']].append(
                            (0, {'delete': read['length']}))
            ...
            if isinstance(piece.get('retain'), dict) and local_reads:
                ...
                for nested in _walk_move_ops(piece['retain']):
                    ...
                    if isinstance(nested_spec, dict) and 'path' not in nested_spec:
                        payload_windows.add((nested_spec['ref'],
                                             nested_spec['start'],
                                             nested_spec['length']))
                alive = set()
                for nested in _walk_move_ops(piece['retain']):
                    ...
                    if not isinstance(nested_spec, dict):
                        continue
                    for window in shared.other_windows.get(
                            nested_spec['ref'], ()):
                        if (nested_spec['ref'], window.start,
                                window.length) not in payload_windows:
                ...
                for path, group in groups.items():
                    ...
                    if isinstance(edited, list):
                        rebased |= _route_into_reads(
                            edited, group, read_buckets,
                            state, shared.taken, alive)
            ...
            for index, run_low, run_high in covered:
                ...
                attributes = op.transform(
                    window.attributes, piece.get('attributes'), priority)
                if isinstance(piece.get('retain'), dict):
                    change = _transform_embed_change(
                        window_change, piece['retain'], priority)
                    if change is None and not attributes:
                        continue
                    routed = {'retain': change if change is not None
                              else run_high - run_low}
                else:
                    if not attributes:
                        continue
                    routed = {'retain': run_high - run_low}
                if attributes:
                    routed['attributes'] = attributes
                if (isinstance(routed.get('retain'), dict)
                        and _payload_has_pastes(routed['retain'])):
                    routed = ('rewrite', routed)  # renumber at expansion
                _deposit(shared, ref, windows, index, run_low - start, routed,
                         local_pastes)
        offset += piece_length
...
def _cut_piece(out, shared, spec, deleted, attributes, change=None):
    ...
    if change is not None and next(_walk_move_ops(change), None) is not None:
        ...
        run_start = sum(span for span, *_ in mapping['segments'])
        if not _covered_runs(shared.other_windows.get(ref, []),
                             run_start, run_start + spec['length']):
            reads = [read for read
                     in shared.other_reads.get(ref, ())
                     if read['unit'] == run_start]
            _drop_self_embed(change, shared, reads)
    if deleted:
        mapping['segments'].append((spec['length'], None, 0, None, None))
        return
    last = out[-1] if out else None
    if (isinstance(last, dict) and op.type(last) == 'cut'
            and last['cut']['ref'] == mapping['current']):
    ...
    mapping['segments'].append(
        (spec['length'], mapping['current'], offset, attributes, change))


def _renumber(operation, state, priority):
    ...
    if 'path' in spec:
        ...
        for span, part_ref, offset, _attrs, change in mapping['segments']:
            if position <= unit < position + span:
                ...
                piece = copy.deepcopy(operation)
                piece['paste'].update(ref=part_ref,
                                      unit=offset + unit - position)
                if change is not None:
                    edited = _navigate_patch(change[next(iter(change))],
                                             spec['path'])
                    if isinstance(edited, list):
                        pieces = []
                        for start, length in _read_spans(
                                edited, spec['start'],
                                spec['start'] + spec['length']):
                            fragment = copy.deepcopy(piece)
                            fragment['paste'].update(start=start,
                                                     length=length)
    ...
    for span, part_ref, offset, attributes, change in mapping['segments']:
        ...
        if low < high and part_ref is not None:
            piece_spec = {'ref': part_ref, 'start': offset + low - position,
                          'length': high - low}
            transformed_change = _transform_embed_change(
                change, spec.get('change'), priority)
            if transformed_change is not None:
                piece_spec['change'] = transformed_change
            piece = {'paste': piece_spec}
            transformed = op.transform(
                attributes, operation.get('attributes'), priority)
            if transformed:
...
def _laid_out(entries, unwrap):
    ...
    for offset, piece in sorted(entries, key=lambda entry: entry[0]):
        ...
        output.append(piece)
        cursor = offset + _input_length(piece)
...
def _renumber_list(items, state, priority):
    ...
    for item in items:
        ...
        spec = operation.get('paste')
        if isinstance(spec, dict) and spec.get('ref') in state:
            pieces = _renumber(operation, state, priority)
            if pieces is not None:
                result.extend(pieces)
                continue
        if isinstance(operation.get('retain'), dict):
            _rewrite_payload(operation['retain'], state, priority)
        if isinstance(operation.get('insert'), dict):
            _rewrite_payload(operation['insert'], state, priority)
        result.append(operation)
    return result


def _expand_markers(out, buckets, read_buckets, cls,
                    state=None, priority=False):
    ...
    if state is not None:
        def unwrap(piece):
            return _routed_entry(piece, state, priority)

    def expand(item):
        if isinstance(item, tuple):
            if item[0] == 'read':
                return _expand_window(item[2], read_buckets.get(item[1], []),
                                      unwrap)
            return _expand_window(item[3],
                                  buckets.get((item[1], item[2]), []), unwrap)
        return None
...
def _payload_cut_sites(retain):
    ...
    sites = []
    pasted = {operation['paste']['ref']
              for operation in _walk_move_ops(retain)
              if isinstance(operation.get('paste'), dict)}
        ...
        if isinstance(sequence, list):
            ...
            for child in sequence:
                if not isinstance(child, dict):
                    continue
                if isinstance(child.get('cut'), dict):
                    if child['cut']['ref'] not in pasted:
                        sites.append((child['cut']['ref'], (*path, 'ops'),
                                      offset, child['cut']['length']))
                    offset += child['cut']['length']
                    continue
                if isinstance(child.get('retain'), dict):
                    inner = child['retain']
                    visit(inner[next(iter(inner))],
                          (*path, 'ops', offset))  # descend a level deeper
                offset += _input_length(child)
        for key, item in value.items():
            if key != 'ops':
                visit(item, (*path, key))

    visit(retain[next(iter(retain))], ())
    return sites
...
def _trash_embed(retain, shared, out):
    ...
    for ref, path, offset, _length in sites:
        shared.trash[ref] = {'ref': trash_ref, 'unit': 0,
...
def _retarget_trashed(delta, trash):
    ...
    for operation in list(_walk_move_ops(ops)):
        ...
        if (isinstance(spec, dict) and spec['ref'] not in cuts
                and spec['ref'] in trash and 'path' not in spec):
            site = trash[spec['ref']]
            spec.update(ref=site['ref'], unit=site['unit'],
                        path=list(site['path']),
                        start=site['offset'] + spec['start'])
...
def _sourced(delta):
    ...
    for operation in _walk_move_ops(list(delta.ops)):
        spec = operation.get('paste')
        if isinstance(spec, dict) and spec['ref'] not in cuts:
            raise ValueError('cannot compose the deletion of an embed '
                             'that still sources a move')
    return delta
...
class MoveDelta:
    ...
    def __init__(self, ops: Ops | 'MoveDelta' | None = None, **attrs: Any) -> None:
        if hasattr(ops, 'ops'):
            ops = ops.ops
        self.ops = ops or []
    ...
    def extend(self, ops: Ops | 'MoveDelta') -> Self:
        if hasattr(ops, 'ops'):
            ops = ops.ops
        if not ops:
            return self
    ...
    def concat(self, other: 'MoveDelta') -> Self:
    ...
    def diff(self, other: 'MoveDelta', cursor: int | None = None) -> Self:
        """
        ...
        if cursor is None:
            return snapshot_diff(self, other)
        return snapshot_diff_at(self, other, cursor)
    ...
    def iter_lines(self, newline: str = '\n'):
        ...
        while it.has_next():
            ...
            if isinstance(current_op.get('insert'), str):
                ...
                nl_index = (op.str_length(suffix[:found])
                            if found >= 0 else -1)
            else:
                nl_index = -1
    ...
    def paste(self, ref: Ref, start: int, length: int, **attrs: Any) -> Self:
        if length <= 0:
            return self
        ...
        if self.ops:
            ...
            if op.type(operation) == op.type(last_op) == 'paste':
                ...
                if (last['ref'] == new['ref']
                        ...
                        and 'change' not in last and 'change' not in new
                    ...
                    return self
            if (op.type(operation) == op.type(last_op) == 'cut'
                    and last_op['cut']['ref'] == operation['cut']['ref']):
    ...
    def compose(self, other: 'MoveDelta') -> 'MoveDelta':
        shared = _COMPOSE_STATE.get()
        if shared is not None:  # a child sequence joining the transaction
            shared.depth += 1
            try:
                return self._compose_with_moves(list(other.ops), shared)
            finally:
                shared.depth -= 1
        check(self)
        check(other)
        return _sourced(self._compose_transaction(
            _renamed(_refs(self.ops), other.ops)))

    def _compose_unchecked(self, other: 'MoveDelta') -> 'MoveDelta':
    ...
    def _compose_transaction(self, other_ops: Ops) -> 'MoveDelta':
        refs = _refs(self.ops) | _refs(other_ops)
        other_cuts = {operation['cut']['ref']
                      for operation in _walk_move_ops(list(other_ops))
                      if isinstance(operation.get('cut'), dict)}
        tables = {}
        trash = {}
        while True:
            shared = ComposeState(tables=tables, taken=set(refs),
                                  cuts=other_cuts, trash=trash)
            token = _COMPOSE_STATE.set(shared)
            try:
                result = self._compose_with_moves(other_ops, shared)
            finally:
                _COMPOSE_STATE.reset(token)
            if not shared.retry:
    ...
    def _compose_with_moves(self, other_ops: Ops, shared: ComposeState) -> 'MoveDelta':
        ...
        while self_it.has_next() or other_it.has_next():
            ...
            if self_it.peek_type() in ('delete', 'cut'):
                ...
                continue
            if other_type == 'cut':
                _consume_cut(other_it.next()['cut'], self_it, out, shared)
                continue
            ...
            if other_op.get('retain') is not None:
                new_op, sealed = _composed_retain(self_op, other_op, length)
                out.append(('sealed', new_op) if sealed else new_op)
            elif (op.type(other_op) == 'delete'
                    and isinstance(self_op.get('retain'), (int, float, dict))):
                if (isinstance(self_op.get('retain'), dict)
                        and _trash_embed(self_op['retain'], shared, out)):
                    continue  # deletion of a sourcing embed became a trash cut
                out.append(other_op)

        def expand(operation):
            if isinstance(operation, tuple):  # sealed: an embed handler
                return [operation[1]]         # already expanded it
            for carrier in ('insert', 'retain'):
                ...
                if isinstance(operation, dict) and isinstance(
                        operation.get(carrier), dict):
                    payload = _expanded_payload(operation[carrier], tables,
                                                shared)
                    if payload is not None:
            ...
            if ref in tables:
                return _expand(operation, tables)
            if ref in shared.cuts:
    ...
    def transform(self, other: 'MoveDelta | int | float', priority: bool = False) -> 'MoveDelta | int | float':
        if isinstance(other, (int, float)):
            return self.transform_position(other, priority)
        shared = _TRANSFORM_STATE.get()
        if shared is not None:  # a child sequence joining the transaction
            return self._transform_with_moves(other, priority, shared)
        check(self)
        check(other)
        return _normalize_orphans(self._transform_transaction(other, priority))

    def _transform_transaction(self, other: 'MoveDelta', priority: bool) -> 'MoveDelta':
        other_reads = {}
        for operation in other.ops:
            if op.type(operation) == 'paste' and 'path' in operation['paste']:
                spec = operation['paste']
                other_reads.setdefault(spec['ref'], []).append(
                    {'unit': spec.get('unit', 0), 'path': tuple(spec['path']),
                     'start': spec['start'], 'length': spec['length'],
        ...
        token = _TRANSFORM_STATE.set(shared)
        try:
            # the post-passes stay inside the transaction: renumbering
            # and overlays may transform transaction fragments through
            # handlers, which must join rather than re-validate.  Marker
            # expansion is deferred past renumbering so edits it routes
            # into windows still land.
            out = self._transform_with_moves(other, priority, shared,
                                             raw=True)
            out = _renumber_list(out, shared.state, priority)
            result = _expand_markers(out, shared.buckets,
                                     shared.read_buckets, self.__class__,
                                     shared.state, priority)
            return _apply_overlays(result, shared, priority)
        finally:
            _TRANSFORM_STATE.reset(token)

    def _transform_with_moves(self, other: 'MoveDelta', priority: bool,
                              shared: TransformState, raw: bool = False):
        ...
        state = shared.state
        taken = shared.taken
        local_pastes = {(o['paste']['ref'], o['paste']['start'])
                        for o in self.ops if op.type(o) == 'paste'}
        window_index = {(ref, span.start): index
                        for ref, spans in shared.self_windows.items()
                        for index, span in enumerate(spans)}
        ...
        for operation in self.ops:
            if op.type(operation) == 'paste' and 'path' in operation['paste']:
                spec = operation['paste']
                key = (spec['ref'], spec.get('unit', 0),
                       tuple(spec['path']), spec['start'])
                local_reads.setdefault(spec['ref'], []).append(
                    {'unit': spec.get('unit', 0), 'path': tuple(spec['path']),
                     'start': spec['start'], 'length': spec['length'],
                     'attributes': operation.get('attributes'), 'key': key})
                read_buckets[key] = []
        ...
        while self_it.has_next() or other_it.has_next():
            self_type = self_it.peek_type()
            other_type = other_it.peek_type()
            if (self_type in ('insert', 'paste')
                    and (priority or other_type not in ('insert', 'paste'))):
                if self_type == 'insert':
                ...
                if 'path' in spec:  # a trash read hosts routed edits too
                    out.append(('read',
                                (spec['ref'], spec.get('unit', 0),
                                 tuple(spec['path']), spec['start']),
                                spec['length']))
                else:
                    out.append(('window', spec['ref'],
                                window_index[(spec['ref'], spec['start'])],
                                spec['length']))
                continue
            if other_type in ('insert', 'paste'):
                out.append(other_it.next())
                continue
            if self_type == 'cut':
                _route_cut(self_it.next()['cut'], other_it, shared, out,
                           priority, local_pastes, local_reads, read_buckets)
                continue
            length = min(self_it.peek_length(), other_it.peek_length())
            self_op = self_it.next(length)
            other_op = other_it.next(length)
            if op.type(other_op) == 'cut':
                _cut_piece(out, shared, other_op['cut'],
                           deleted=bool(self_op.get('delete')),
                           attributes=self_op.get('attributes'),
                           change=self_op['retain']
                           if isinstance(self_op.get('retain'), dict) else None)
                ...
                out.append(_transformed_retain(self_op, other_op, length, priority))

        if raw:
            shared.read_buckets = read_buckets
            return out
        return _expand_markers(out, shared.buckets, read_buckets,
                               self.__class__)
    ...
    def transform_position(self, index: int, priority: bool = False) -> int:
        ...
        if followed is not None:
            ...
            for operation in self.ops:
                ...
                output_position += _output_length(operation)
        position = index
        passed = 0
        for operation in self.ops:
            if passed > position:
    ...
    def lower(self, base: 'MoveDelta') -> 'MoveDelta':
    ...
    def invert(self, base: 'MoveDelta') -> 'MoveDelta':
        ...
        if owner:
            ...
            for operation in _walk_move_ops(list(self.ops), skip_inserts=True):
                ...
                if isinstance(spec, dict) and 'path' not in spec:
                    windows.setdefault(spec['ref'], []).append(Window(
                        spec['start'], spec['length'],
                        operation.get('attributes'), spec.get('change')))
            inverse_refs = {}
            taken = set()
            for ref in sorted(windows):
                windows[ref].sort(key=lambda span: span.start)
                for index, window in enumerate(windows[ref]):
                    inverse_refs[(ref, window.start)] = _fresh_ref(
                        ref if index == 0 else f'{ref}:{index}', taken)
            shared = InvertState(windows=windows, inverse_refs=inverse_refs)
            token = _INVERT_STATE.set(shared)
        try:
            return self._invert_with_moves(base, shared)
        finally:
            if owner:
                _INVERT_STATE.reset(token)

    def _invert_with_moves(self, base: 'MoveDelta', shared: InvertState) -> 'MoveDelta':
        ...
        for operator in self.ops:
            ...
            if kind == 'insert':
                ...
                for window in windows.get(spec['ref'], []):
                    ...
                    offset = 0
                    for base_op in base[base_index + start:
                                        base_index + start + length]:
                        piece_length = op.length(base_op)
                        piece_spec = {'ref': inverse_ref, 'start': offset,
                                      'length': piece_length}
                        if change is not None:
                            revert_change = _invert_embed_change(
                                change, base_op.get('insert'))
                            if revert_change is not None:
                                piece_spec['change'] = revert_change
                        piece = {'paste': piece_spec}
                        revert = attributes and op.invert(
                            attributes, base_op.get('attributes'))
                        if revert:
                            piece['attributes'] = revert
                        inverted.push(piece)
                        offset += piece_length
                    position = start + length
                for base_op in base[base_index + position:
                                    base_index + spec['length']]:
                    inverted.push(base_op)
                base_index += spec['length']
            elif (isinstance(operator.get('retain'), (int, float))
                    and operator.get('attributes') is None):
