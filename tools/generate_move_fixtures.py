...
class CellHandler:
    ...
    def transform(a, b, priority, context):
        return {"ops": MoveDelta(list(a["ops"])).transform(MoveDelta(list(b["ops"])), priority, _context=context).ops}
...
def main(out_dir):
    ...
    for _ in range(120):
        ...
        if kind < 0.3:
            ...
            base = MoveDelta().insert("AB").insert(T.cell("Hello")).insert("CD")
        ...
        compose_cases.append(case(first=move, second=edit, expected=move.compose(edit)))

        concurrent = T.random_ordinary(rng, len(base))
        for priority in (True, False):
            transform_cases.append(
                case(left=move, right=concurrent, priority=priority, expected=move.transform(concurrent, priority))
            )
        other_move = T.random_move(rng, len(base), ref="z")
        transform_cases.append(
            case(left=move, right=other_move, priority=True, expected=move.transform(other_move, True))
        )

        invert_cases.append(case(delta=move, base=base, expected=move.invert(base)))

        for index in range(0, len(base) + 1, max(1, len(base) // 3)):
            position_cases.append(
                case(delta=move, position=index, priority=False, expected=move.transform_position(index))
            )
    ...
    invert_cases.append(case(delta=read, base=doc, expected=read.invert(doc)))
    for coordinate in [(1,), (4,), (2, "ops", 1), (2, "ops", 4)]:
        coordinate_cases.append(
            case(
                ...
                expected=(
                    list(transform_coordinate(read, coordinate))
    ...
    apply_cases.append(case(doc=emoji_doc, delta=swap, expected=T.apply(emoji_doc, swap)))

    out = Path(out_dir)
    out.mkdir(parents=True, exist_ok=True)
    for name, cases in [
        ("moves-apply", apply_cases),
