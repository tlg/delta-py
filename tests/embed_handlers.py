...
def _validate_move(move, block_count):
    ...
    if move["index"] + move["count"] > block_count:
        raise ValueError(f"move source out of range: {move['index']} + {move['count']} > {block_count}")
    if move["before"] > block_count:
        raise ValueError(f"move destination out of range: {move['before']} > {block_count}")
...
def apply_move(blocks, move):
    ...
    if normalized is None:
        return list(blocks)
    index, count, before = (normalized["index"], normalized["count"], normalized["before"])
...
def _next_move_cursor(move):
    insert_at = move["before"] if move["before"] < move["index"] else move["before"] - move["count"]
...
def diff_to_moves(from_items, to_items):
    ...
    for target_index in range(len(to_items)):
        ...
        count = 1
        while (
            target_index + count < len(to_items)
            and source_index + count < len(working)
            and working[source_index + count] == to_items[target_index + count]
        ):
            count += 1
        move = normalize_move(resolve_move(source_index, count, target_index), len(working))
        if move is None:
            continue
        moves.append(move)
    ...
    for block_op in ops:
        if _is_move_op(block_op):
            resolved = normalize_move(
                resolve_move(cursor, block_op["move"]["count"], block_op["move"]["before"]), len(current)
            )
            if resolved is None:
                continue
        ...
        if cursor > len(current):
            raise ValueError(f"block cursor out of range: {cursor} > {len(current)}")
    return entries
...
class BlockDelta:
    ...
    def from_moves(cls, moves):
        ...
        for move in moves:
            if move["index"] < cursor:
                raise ValueError(
                    f"move sequence is not representable from the current cursor: {move['index']} < {cursor}"
                )
    ...
    def chop(self):
        last_op = self.ops[-1] if self.ops else None
        if last_op is not None and not _is_move_op(last_op) and last_op.get("retain", 0) > 0:
            self.ops.pop()
        return self

    def resolve(self, block_count):
        return [entry["resolved"] for entry in _resolve_block_ops(self.ops, block_count)]
...
def apply_change(document, change, newline="\n", context=None):
    after_delta = document.compose(change.delta, _context=context)
    if not change.block_delta.ops:
        return after_delta
    result = Delta()
    for block in change.block_delta.apply(_split_line_blocks(after_delta, newline)):
...
def transform_change(left, right, document, priority=False, newline="\n", context=None):
    if not left.block_delta.ops and not right.block_delta.ops:
        return Change(left.delta.transform(right.delta, priority, _context=context), BlockDelta())
    if not left.delta.ops and not right.delta.ops:
        block_count = len(_split_line_blocks(document, newline))
        return Change(Delta(), left.block_delta.transform(right.block_delta, block_count, priority))
    raise ValueError("transform_change: mixed delta/blockDelta transforms are not supported by the test kernel")
...
def _axis_items_from_ops(ops):
    return [
        {"id": axis_op["insert"]["id"], "op": axis_op} for axis_op in ops if isinstance(axis_op.get("insert"), dict)
    ]
...
def _axis_ops_from_doc(doc):
    ...
    for line, attributes, _index in doc.iter_lines():
        line_id = "".join(line_op["insert"] for line_op in line.ops if isinstance(line_op.get("insert"), str))
        if not line_id:
            continue
...
def _canonical_cell_doc(ops=None):
    ...
    if last is None or not isinstance(last.get("insert"), str) or not last["insert"].endswith(EMPTY_TABLE_LINE):
...
def _change_from_spec(spec=None):
    spec = spec or {}
    return Change(
        Delta(copy.deepcopy(spec.get("delta") or [])), BlockDelta(copy.deepcopy(spec.get("blockDelta") or []))
    )
...
def _apply_axis_change(base_ops, spec=None, context=None):
    if not spec:
        return [dict(item["op"]) for item in _axis_items_from_ops(base_ops)]
    doc = apply_change(_axis_doc_from_ops(base_ops), _change_from_spec(spec), context=context)
    return _axis_ops_from_doc(doc)


def _apply_cell_patch(base_cell, patch, context=None):
    base_cell = base_cell or {}
    patch = patch or {}
    content = _canonical_cell_doc(base_cell.get("content") or [])
    if patch.get("change"):
        content = apply_change(content, _change_from_spec(patch["change"]), context=context)
    attributes = op.compose(base_cell.get("attributes"), patch.get("attributes"), False)
    return _compact_cell_data(content, attributes)


def _diff_cell_patch(base_cell, target_cell, context=None):
    base_cell = base_cell or {}
    target_cell = target_cell or {}
    content_change = _change_spec_from_change(
        Change(
            _canonical_cell_doc(base_cell.get("content") or []).diff(
                _canonical_cell_doc(target_cell.get("content") or []), _context=context
            ),
            BlockDelta(),
        )
    )
    attributes = op.diff(base_cell.get("attributes"), target_cell.get("attributes"))
...
def _same_axis_shape(base_ops, target_ops):
    ...
    if len(set(base_ids)) != len(base_ids) or len(set(target_ids)) != len(target_ids):
        return False
    if sorted(base_ids) != sorted(target_ids):
        return False
    target_by_id = {item["id"]: item["op"] for item in target}
    return all(item["op"].get("attributes") == target_by_id[item["id"]].get("attributes") for item in base)
...
def _build_post_delta_axis_ops(base_ops, target_ops):
    ...
    for item in target:
        if item["id"] in shared_ids:
            inserted_before[item["id"]] = inserted_before.get(item["id"], []) + pending_inserted
            pending_inserted = []
            continue
...
def _diff_axis_change(base_ops, target_ops, context=None):
    base = _axis_items_from_ops(base_ops)
    target = _axis_items_from_ops(target_ops)
    if _same_axis_shape(base_ops, target_ops):
        moves = diff_to_moves([item["id"] for item in base], [item["id"] for item in target])
        return _change_spec_from_change(Change(Delta(), BlockDelta.from_moves(moves)))
    ...
    if post_delta_ops is not None:
        return _change_spec_from_change(
            Change(
                _axis_doc_from_ops(base_ops).diff(_axis_doc_from_ops(post_delta_ops), _context=context),
                BlockDelta.from_moves(
                    diff_to_moves(
                        [item["id"] for item in _axis_items_from_ops(post_delta_ops)], [item["id"] for item in target]
                    )
                ),
            )
        )
    return _change_spec_from_change(
        Change(_axis_doc_from_ops(base_ops).diff(_axis_doc_from_ops(target_ops), _context=context), BlockDelta())
    )
...
def _apply_table_patch(base, patch, context=None):
    rows = _apply_axis_change(base.get("rows") or [], patch.get("rows"), context)
    columns = _apply_axis_change(base.get("columns") or [], patch.get("columns"), context)
    ...
    for identity, cell_patch in (patch.get("cells") or {}).items():
        ...
        if row_id not in valid_rows or column_id not in valid_columns:
            ...
            continue
        cell = _apply_cell_patch((base.get("cells") or {}).get(identity), cell_patch, context)
        if cell:
            cells[identity] = cell
        else:
            cells.pop(identity, None)
...
def _diff_table(base, target, context=None):
    rows = _diff_axis_change(base.get("rows") or [], target.get("rows") or [], context)
    columns = _diff_axis_change(base.get("columns") or [], target.get("columns") or [], context)
    cells = {}
    identities = dict.fromkeys(list(base.get("cells") or {}) + list(target.get("cells") or {}))
    for identity in identities:
        patch = _diff_cell_patch(
            (base.get("cells") or {}).get(identity), (target.get("cells") or {}).get(identity), context
        )
        if patch:
...
def _transform_nested_change(left, right, document, priority, context):
    if not right:
        return None
    if not left:
        return right
    return _change_spec_from_change(
        transform_change(_change_from_spec(left), _change_from_spec(right), document, priority, context=context)
    )
...
def _compose_table_patch(left, right, context):
    base = _clone_table_data(left["base"])
    middle = _apply_table_patch(base, left, context)
    final = _apply_table_patch(middle, right, context)
    return _diff_table(base, final)


def _transform_table_patch(applied, other, priority, context):
    base = _clone_table_data(applied["base"])
    base_after_applied = _apply_table_patch(base, applied)

    rows_prime = _transform_nested_change(
        applied.get("rows"), other.get("rows"), _axis_doc_from_ops(base.get("rows") or []), priority, context
    )
    columns_prime = _transform_nested_change(
        applied.get("columns"), other.get("columns"), _axis_doc_from_ops(base.get("columns") or []), priority, context
    )
    cells = {}
    identities = dict.fromkeys(
        list(base.get("cells") or {}) + list(applied.get("cells") or {}) + list(other.get("cells") or {})
    )
    ...
    for identity in identities:
        ...
        change = _transform_nested_change(
            applied_patch.get("change"),
            other_patch.get("change"),
            _canonical_cell_doc(base_cell.get("content") or []),
            priority,
            context,
        )
        attributes = _transform_cell_attributes(
            applied_patch.get("attributes"), other_patch.get("attributes"), priority
        )
    ...
    final = _apply_table_patch(
        base_after_applied,
        _compact_table_patch(
            {
...
def _invert_table_patch(change, base, context):
    final = _apply_table_patch(base, change)
    return _diff_table(final, base)


def _table_stream_paths(value):
    """Delta streams structurally owned by a table value or patch."""
    ...
    for axis in ("rows", "columns"):
        ...
        if isinstance(spec, list):
            yield (axis,)
        elif isinstance(spec, dict) and isinstance(spec.get("delta"), list):
            yield axis, "delta"
    for identity, cell in (value.get("cells") or {}).items():
        if isinstance(cell.get("content"), list):
            yield "cells", identity, "content"
        change = cell.get("change")
        if isinstance(change, dict) and isinstance(change.get("delta"), list):
            yield "cells", identity, "change", "delta"


class TableHandler:
    stream_paths = staticmethod(_table_stream_paths)

    @staticmethod
    def apply(value, change, context):
        if _is_table_patch(value) or not _is_table_patch(change):
            raise ValueError("table-embed apply expects a patch over a table value")
        return _apply_table_patch(value, change, context)

    @staticmethod
    def compose(a, b, context):
        if not _is_table_patch(a) or not _is_table_patch(b):
            raise ValueError("table-embed compose expects two table patches")
        return _compose_table_patch(a, b, context)

    @staticmethod
    def diff(a, b, context):
        return _diff_table(a, b, context)

    @staticmethod
    def transform(a, b, priority, context):
        if not _is_table_patch(a) or not _is_table_patch(b):
            raise ValueError("table-embed transform expects self-contained table patches")
        return _transform_table_patch(a, b, priority, context)

    @staticmethod
    def invert(change, base, context):
        if not _is_table_patch(change) or _is_table_patch(base):
            raise ValueError("table-embed invert expects change patch over a table document")
        return _invert_table_patch(change, base, context)
...
class DeltaHandler:
    ...
    def apply(a, b, context):
        return DeltaHandler.compose(a, b, context)

    @staticmethod
    def compose(a, b, context):
        return Delta(a).compose(Delta(b), _context=context).ops

    @staticmethod
    def diff(a, b, context):
        return Delta(a).diff(Delta(b), _context=context).ops

    @staticmethod
    def transform(a, b, priority, context):
        return Delta(a).transform(Delta(b), priority, _context=context).ops

    @staticmethod
    def invert(a, b, context):
        return Delta(a).invert(Delta(b), _context=context).ops
