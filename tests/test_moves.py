...
from delta import Delta, MoveDelta, check, op
...
def test_builders():
    ...
    assert delta.ops == [
        {"cut": {"ref": "r", "length": 3}},
        {"retain": 2},
        {"paste": {"ref": "r", "start": 0, "length": 3}, "attributes": {"bold": True}},
    ]

    change = {"cell": {"ops": [{"delete": 2}]}}
    changed = MoveDelta().cut("e", 1).paste("e", 0, 1, change)
    assert changed.ops == [
        {"cut": {"ref": "e", "length": 1}},
        {"paste": {"ref": "e", "start": 0, "length": 1, "change": change}},
    ]


def test_paste_builder_rejects_multi_unit_embed_change():
    with pytest.raises(ValueError, match="must address one embed"):
        MoveDelta().paste("e", 0, 2, change={"cell": {"ops": []}})
...
def test_push_owns_flat_and_nested_operations():
    operation = {"retain": 1}
    flat = MoveDelta().push(operation)
    operation["retain"] = 2
    assert flat.ops == [{"retain": 1}]

    change = {"cell": {"ops": []}}
    nested = MoveDelta().paste("r", 0, 1, change=change)
    change["cell"]["ops"].append({"insert": "later"})
    assert nested.ops[0]["paste"]["change"]["cell"]["ops"] == []
...
def test_check_rejects_invalid_moves():
    ...
    # ordinary refs are transfers, not aliases for delete; compiler-created
    # trash cuts retain a matching path-addressed trash-read paste
    with pytest.raises(ValueError, match="has no paste"):
        check(MoveDelta().cut("r", 3))


def test_check_matches_typescript_integer_semantics():
    integral_floats = MoveDelta(
        [
            {"cut": {"ref": "r", "length": 2.0}},
            {"paste": {"ref": "r", "start": 0.0, "length": 2.0}},
        ]
    )
    assert check(integral_floats) is integral_floats

    for length in (True, 1.5, float("inf"), float("nan")):
        with pytest.raises(ValueError, match="positive integer length"):
            check(MoveDelta([{"cut": {"ref": "r", "length": length}}]))

    for field, value in (("start", False), ("start", 0.5), ("length", True), ("length", 1.5)):
        paste = {"ref": "r", "start": 0, "length": 2}
        paste[field] = value
        with pytest.raises(ValueError, match="non-negative integer start"):
            check(MoveDelta([{"cut": {"ref": "r", "length": 2}}, {"paste": paste}]))


def test_ref_names_do_not_collide_with_internal_pending_state():
    ref = "\x00pending:user"
    move = MoveDelta().cut(ref, 1).retain(2).paste(ref, 0, 1)
    composed = move.compose(MoveDelta().retain(3))
    check(composed)
    assert composed == move
...
def test_apply_moves_embeds():
    ...
    assert apply(base, move).ops == [{"insert": "xy"}, {"insert": {"image": "a.png"}}]
...
def test_compose_format_lands_on_paste_attributes():
    ...
    assert composed.ops == [
        {"cut": {"ref": "r", "length": 10}},
        {"retain": 3},
        {"paste": {"ref": "r", "start": 0, "length": 2}, "attributes": {"bold": True}},
        {"paste": {"ref": "r", "start": 2, "length": 8}},
    ]
    base = doc("ABCDEFGHIJxyz")
    assert apply(base, composed) == apply(base, move).compose(edit)
...
def test_compose_cut_over_a_paste_chains_the_reference():
    ...
    second = MoveDelta().paste("s", 0, 3).retain(2).cut("s", 3).retain(1)
    composed = first.compose(second)
    base = doc("ABCDEF")
...
def fig_handler():
    class FigHandler:
        @staticmethod
        def apply(a, b, _context):
            merged = {**(a or {}), **(b or {})}
            return {k: v for k, v in merged.items() if v is not None}

        @staticmethod
        def compose(a, b, _context):
            return {**(a or {}), **(b or {})}

        @staticmethod
        def transform(a, b, priority, _context):
            if not priority:
                return b
            return {k: v for k, v in (b or {}).items() if k not in (a or {})}

        @staticmethod
        def invert(change, base, _context):
            return {k: base.get(k) for k in (change or {}) if base.get(k) != change[k]}
...
def test_handler_apply_is_distinct_from_compose(fig_handler):
    base = MoveDelta().insert({"fig": {"keep": 1, "drop": 2}})
    applied = base.compose(MoveDelta().retain({"fig": {"drop": None}}))
    assert applied == MoveDelta().insert({"fig": {"keep": 1}})

    composed = MoveDelta().retain({"fig": {"drop": None}}).compose(MoveDelta().retain({"fig": {"add": 3}}))
    assert composed == MoveDelta().retain({"fig": {"drop": None, "add": 3}})


def test_recursive_context_is_private_and_operation_specific():
    wrong = {"_kind": "wrong"}
    calls = {
        "compose": lambda: MoveDelta().compose(MoveDelta(), _context=wrong),
        "transform": lambda: MoveDelta().transform(MoveDelta(), _context=wrong),
        "invert": lambda: MoveDelta().invert(MoveDelta(), _context=wrong),
        "diff": lambda: MoveDelta().diff(MoveDelta(), _context=wrong),
    }
    for operation, call in calls.items():
        with pytest.raises(TypeError, match=operation):
            call()
    with pytest.raises(TypeError):
        MoveDelta().compose(MoveDelta(), wrong)
...
def test_compose_moving_an_embed_change(fig_handler):
    ...
    assert composed.ops == [
        ...
        {"cut": {"ref": "m", "length": 1}},
        {"retain": 1},
        {"paste": {"ref": "m", "start": 0, "length": 1, "change": {"fig": {"w": 2}}}},
    ]
    base = MoveDelta().insert("a").insert({"fig": {"v": 1}}).insert("b")
    assert apply(base, composed) == apply(base, first).compose(second)
    assert apply(base, composed).ops == [{"insert": "ab"}, {"insert": {"fig": {"v": 1, "w": 2}}}]
...
def test_invert_reverts_a_paste_change(fig_handler):
    ...
    move = MoveDelta(
        [
            {"cut": {"ref": "m", "length": 1}},
            {"retain": 2},
            {"paste": {"ref": "m", "start": 0, "length": 1, "change": {"fig": {"v": 9, "w": 2}}}},
        ]
    )
...
def test_transform_concurrent_embed_change_and_move_converge(fig_handler):
    ...
    for move_wins in (True, False):
        ...
        assert apply(base, move).compose(edit_prime) == apply(base, edit).compose(move_prime), move_wins
...
def test_invert_preserves_an_empty_attribute_retain_position():
    base = doc("ABC")
    change = MoveDelta([{"retain": 2, "attributes": {}}, {"insert": "x"}])
    inverse = change.invert(base)
    assert inverse.ops == [{"retain": 2}, {"delete": 1}]
    assert apply(base, change).compose(inverse) == base
...
def test_invert_reverts_paste_attribute_patches():
    ...
    inverted = move.invert(base)
    assert inverted.ops == [
        {"paste": {"ref": "r", "start": 0, "length": 2}, "attributes": {"bold": True, "i": None}},
        {"retain": 2},
        {"cut": {"ref": "r", "length": 2}},
    ]
    assert apply(base, move).compose(inverted) == base
...
def test_invert_splits_avoid_a_shadowing_literal_ref():
    # inverting ref 'r' with two windows wants the names r and r:1 —
    # while a literal ref named 'r:1' also exists.  A collision would
    # merge the two capture streams on undo.
    base = doc("ABCDEFGH")
    move = MoveDelta().cut("r", 4).cut("r:1", 2).retain(2).paste("r", 0, 1).paste("r:1", 0, 2).paste("r", 3, 1)
    inverted = move.invert(base)
    refs = [o["cut"]["ref"] for o in inverted.ops if "cut" in o]
...
def test_transform_convergence_simple():
    ...
    for edit in (
        ...
        for move_wins in (True, False):
            edit_prime = move.transform(edit, move_wins)
            move_prime = edit.transform(move, not move_wins)
            assert apply(base, move).compose(edit_prime) == apply(base, edit).compose(move_prime), (edit, move_wins)
...
def test_transform_concurrent_same_source_moves_rebase():
    ...
    assert apply(base, winner).compose(loser_prime) == apply(base, loser).compose(winner_prime) == doc("ADEFBC")
...
def test_transform_concurrent_overlapping_moves_converge():
    ...
    for a_wins in (True, False):
        b_prime = a.transform(b, a_wins)
        a_prime = b.transform(a, not a_wins)
        assert apply(base, a).compose(b_prime) == apply(base, b).compose(a_prime), a_wins


def test_transform_two_cuts_under_one_concurrent_cut_converge():
    base = doc("ABCDEF")
    # self moves BC and E away separately; other moves ABCDE wholesale.
    # The two contested stretches belong to different cuts of self, so
    # their re-claims must not share a re-emitted part.
    two = MoveDelta().retain(1).cut("x", 2).retain(1).cut("y", 1).retain(1).paste("x", 0, 2).paste("y", 0, 1)
    one = MoveDelta().cut("b", 5).retain(1).paste("b", 0, 5)
    for two_wins in (True, False):
        one_prime = two.transform(one, two_wins)
        two_prime = one.transform(two, not two_wins)
        assert apply(base, two).compose(one_prime) == apply(base, one).compose(two_prime), two_wins


def test_transform_renamed_parts_avoid_literal_refs():
    # splitting the claim of ref 'b' wants a part named b:1 — while a
    # literal ref b:1 also exists; a collision merges unrelated streams
    base = doc("ABCDE")
    mover = MoveDelta().cut("x", 2).retain(3).paste("x", 1, 1).paste("x", 0, 1)
    other = MoveDelta().cut("b", 2).cut("b:1", 1).retain(2).paste("b", 0, 2).paste("b:1", 0, 1)
    for wins in (True, False):
        other_prime = mover.transform(other, wins)
        mover_prime = other.transform(mover, not wins)
        refs = [o["cut"]["ref"] for o in other_prime.ops if "cut" in o]
        assert len(refs) == len(set(refs)), wins
        assert apply(base, mover).compose(other_prime) == apply(base, other).compose(mover_prime), wins
...
def random_move(rng, length, ref="r"):
    ...
    for order, window in enumerate(windows, 1):
        target = rng.choice([q for q in range(length + 1) if q <= source or q >= source + size])
...
def test_fuzz_transform_convergence():
    ...
    for _ in range(400):
        ...
        for move_wins in (True, False):
            edit_prime = move.transform(edit, move_wins)
            move_prime = edit.transform(move, not move_wins)
            assert apply(base, move).compose(edit_prime) == apply(base, edit).compose(move_prime), (
                base.ops,
                move.ops,
                edit.ops,
                move_wins,
...
def test_fuzz_transform_move_vs_move_convergence():
    ...
    for _ in range(400):
        ...
        for first_wins in (True, False):
            ...
            assert apply(base, first).compose(second_prime) == apply(base, second).compose(first_prime), (
                base.ops,
                first.ops,
                second.ops,
                first_wins,
...
def doc_units(document):
    ...
    for operation in document.ops:
        ...
        units.extend("t" * op.str_length(insert) if isinstance(insert, str) else "e")
    return units
...
def random_fig_doc(rng):
    ...
    for index in range(rng.randint(2, 5)):
        if rng.random() < 0.35:
            ...
            result.insert("".join(rng.choice(LETTERS) for _ in range(rng.randint(1, 3))))
    return result
...
def random_fig_move(rng, units, ref="r"):
    ...
    for operation in move.ops:
        ...
        if spec and spec["length"] == 1 and rng.random() < 0.6 and units[source + spec["start"]] == "e":
...
def test_fuzz_embed_changes_with_moves(fig_handler):
    ...
    for _ in range(300):
        ...
        if len(after_edit) >= 2:
            ...
            assert after_edit.compose(chased) == apply(base, MoveDelta(edit.ops).compose(chased))
        ...
        for pair in ((move, edit), (move, second)):
            for wins in (True, False):
                ...
                assert apply(base, pair[0]).compose(right_prime) == apply(base, pair[1]).compose(left_prime), (
                    base.ops,
                    pair[0].ops,
                    pair[1].ops,
                    wins,
...
def cell_handler():
    class CellHandler:
        ...
        def apply(a, b, context):
            return CellHandler.compose(a, b, context)

        @staticmethod
        def compose(a, b, context):
            return {"ops": MoveDelta(list(a["ops"])).compose(MoveDelta(list(b["ops"])), _context=context).ops}

        @staticmethod
        def transform(a, b, priority, context):
            return {
                "ops": MoveDelta(list(a["ops"])).transform(MoveDelta(list(b["ops"])), priority, _context=context).ops
            }

        @staticmethod
        def invert(change, base, context):
            return {"ops": MoveDelta(list(change["ops"])).invert(MoveDelta(list(base["ops"])), _context=context).ops}

        @staticmethod
        def diff(a, b, context):
            return {"ops": MoveDelta(list(a["ops"])).diff(MoveDelta(list(b["ops"])), _context=context).ops}
...
def test_apply_cell_to_root_move(cell_handler):
    ...
    assert apply(base, move).ops == [{"insert": "AB"}, {"insert": {"cell": {"ops": []}}}, {"insert": "CHelloD"}]
    ...
    assert apply(base, leftward).ops == [{"insert": "HelloAB"}, {"insert": {"cell": {"ops": []}}}, {"insert": "CD"}]
...
def test_apply_root_to_cell_move(cell_handler):
    ...
    assert apply(base, move).ops == [{"insert": {"cell": {"ops": [{"insert": "HelloAB"}]}}}, {"insert": "CD"}]
...
def test_compose_deleting_a_sourcing_embed_uses_the_trash(cell_handler):
    ...
    assert composed.ops == [
        ...
        {"retain": 1},
        {"paste": {"ref": "trash", "unit": 0, "path": ["ops"], "start": 1, "length": 3}},
    ]
...
def test_compose_trash_ref_does_not_collide_with_a_user_ref(cell_handler):
    base = MoveDelta().insert("QX").insert(cell("AB")).insert("Y")
    first = MoveDelta(
        [
            {"cut": {"ref": "trash", "length": 1}},
    ...
    later = MoveDelta().retain(2).delete(1)
    composed = first.compose(later)
    refs = [operation["cut"]["ref"] for operation in composed.ops if "cut" in operation]
    assert len(refs) == len(set(refs))
    assert apply(base, composed) == apply(base, first).compose(later)
    check(composed)


def test_compose_cut_preserves_a_source_inside_its_retained_embed(cell_handler):
    base = MoveDelta().insert("X").insert(cell("AB")).insert("Y")
    first = MoveDelta(
        [
            {"retain": 1},
            cell_patch({"cut": {"ref": "m", "length": 2}}),
    ...
    later = MoveDelta().cut("b", 3).retain(2).paste("b", 0, 1)
    composed = first.compose(later)
    assert apply(base, composed) == apply(base, first).compose(later) == doc("ABX")
    assert any(operation.get("paste", {}).get("path") == ["ops"] for operation in composed.ops)
    check(composed)


def test_compose_rejects_deleting_an_inserted_embed_that_still_sources_a_move(cell_handler):
    first = MoveDelta(
        [
            {"insert": {"cell": {"ops": [{"cut": {"ref": "m", "length": 1}}]}}},
            {"paste": {"ref": "m", "start": 0, "length": 1}},
        ]
    )
    with pytest.raises(ValueError, match="deletion of an embed that still sources a move"):
        first.compose(MoveDelta().delete(1))
...
def test_transform_cell_edit_routes_to_root_paste_site(cell_handler):
    ...
    edit = MoveDelta([{"retain": 2}, cell_patch({"retain": 1}, {"retain": 2, "attributes": {"bold": True}})])
    routed = move.transform(edit, True)
...
def test_transform_cross_level_convergence(cell_handler):
    ...
    edits = (
        ...
        MoveDelta([{"retain": 2}, cell_patch({"retain": 1}, {"delete": 2})]),
        MoveDelta([{"retain": 2}, cell_patch({"retain": 2}, {"insert": "zz"})]),
...
def cell_length(document):
    for operation in document.ops:
        ...
        if isinstance(insert, dict) and "cell" in insert:
            return sum(op.str_length(o["insert"]) if isinstance(o["insert"], str) else 1 for o in insert["cell"]["ops"])
    return None
...
def random_cross_move(rng, document, ref="r"):
    ...
    if kind == "inside-cell":
        ...
        return MoveDelta().retain(embed_position).push({"retain": {"cell": {"ops": child.ops}}})
    if kind == "cell-to-root":
        size = rng.randint(1, min(3, inner))
        source = rng.randint(0, inner - size)
        target = rng.randint(0, root_length)
        patch = cell_patch({"retain": source} if source else {"retain": 0}, {"cut": {"ref": ref, "length": size}})
        patch["retain"]["cell"]["ops"] = [o for o in patch["retain"]["cell"]["ops"] if o != {"retain": 0}]
        events = [(embed_position, 0, patch), (target, 1, {"paste": {"ref": ref, "start": 0, "length": size}})]
        events.sort(key=lambda event: (event[0], rng.random()))
        delta = MoveDelta()
        position = 0
    ...
    events = [
        (source, 0, {"cut": {"ref": ref, "length": size}}),
...
def test_fuzz_cross_level_moves(cell_handler):
    ...
    for _ in range(250):
        ...
        later = (
            random_two_level_edit(rng, after)
            if cell_length(after) is not None
            else random_ordinary(rng, len(doc_units(after)))
        )
        assert after.compose(later) == apply(base, move.compose(later)), (move.ops, later.ops)
...
def test_fuzz_multi_ref_deltas_and_transform_output_inverts():
    ...
    for _ in range(300):
        ...
        for combined_wins in (True, False):
            ...
            assert one.compose(other_prime.invert(apply(base, combined))) == apply(base, combined)
...
def test_coordinate_root_caret_matches_transform_position():
    ...
    for index in range(7):
        assert transform_coordinate(move, (index,)) == (move.transform_position(index),)
...
def test_coordinate_unit_dies_with_its_embed():
    assert transform_coordinate(MoveDelta().retain(2).delete(1), (2, "ops", 1)) is None
...
def deep_base():
    ...
    middle = {"cell": {"ops": [{"insert": "mno"}, {"insert": inner}, {"insert": "pq"}]}}
        ...
        MoveDelta(
            [
                {"retain": 2},
                {
                    "retain": {
                        "cell": {
                            "ops": [{"retain": 3}, {"retain": {"cell": {"ops": [{"cut": {"ref": ref, "length": 4}}]}}}]
                        }
                    }
                },
                {"retain": 1},
                        ...
                        "cell": {
                            "ops": [
                                {"retain": 3},
                                {
                                    "retain": {
                                        "cell": {
                                            "ops": [{"retain": 4}, {"paste": {"ref": ref, "start": 0, "length": 2}}]
                                        }
                                    }
                                },
                            ]
                        ...
                        "cell": {
                            "ops": [
                                {"cut": {"ref": ref, "length": 3}},
                                {
                                    "retain": {
                                        "cell": {
                                            "ops": [{"retain": 4}, {"paste": {"ref": ref, "start": 0, "length": 3}}]
                                        }
                                    }
                                },
                            ]
                    ...
                    "retain": {
                        "cell": {
                            "ops": [
                                ...
                                {"retain": 2},
                                {"retain": {"cell": {"ops": [{"cut": {"ref": ref, "length": 4}}]}}},
                            ]
                        }
                    }
                },
...
def test_nested_trash_paths_use_stream_offsets_not_list_indices(cell_handler):
    inner = cell("AB")
    outer = {"cell": {"ops": [{"insert": "abc"}, {"insert": inner}, {"insert": "Y"}]}}
    base = MoveDelta().insert("X").insert(outer).insert("Z")
    read = MoveDelta(
        [
            {"cut": {"ref": "b", "length": 3}},
            {"paste": {"ref": "b", "start": 0, "length": 2, "unit": 1, "path": ["ops", 3, "ops"]}},
            {"paste": {"ref": "b", "start": 0, "length": 1}},
        ]
    )
    edit = MoveDelta(
        [{"retain": 1}, cell_patch({"retain": 3}, cell_patch({"retain": 1, "attributes": {"bold": True}}))]
    )
    assert apply(base, read) == doc("ABX")
    for priority in (True, False):
        edit_prime = read.transform(edit, priority)
        read_prime = edit.transform(read, not priority)
        assert apply(base, read).compose(edit_prime) == apply(base, edit).compose(read_prime)
...
def test_deep_coordinates_follow_moves(cell_handler):
    move_out, move_in, move_down, _move_up = deep_moves("m")
...
def test_transform_is_total_over_trash_reads(cell_handler):
    ...
    edits = (
        MoveDelta().insert("X"),
        MoveDelta().retain(4).delete(1),
        cell_edit := MoveDelta([{"retain": 2}, cell_patch({"retain": 1}, {"delete": 1})]),
        MoveDelta([{"retain": 2}, cell_patch({"retain": 2}, {"retain": 2, "attributes": {"bold": True}})]),
    ...
    for edit in edits:
        for wins in (True, False):
            ...
            assert apply(base, composed).compose(right_prime) == apply(base, edit).compose(left_prime), (edit.ops, wins)
...
def test_compose_retargets_base_backed_trash_reads(cell_handler):
    ...
    read = MoveDelta(
        [
            ...
            {"retain": 1},
            {"paste": {"ref": "t", "unit": 2, "path": ["ops"], "start": 0, "length": 5}},
        ]
    )
...
def test_transform_handles_moves_riding_paste_changes(cell_handler):
    ...
    inner = MoveDelta(
        [
            {"retain": 4},
            cell_patch(
                {"cut": {"ref": "m", "length": 2}}, {"retain": 3}, {"paste": {"ref": "m", "start": 0, "length": 2}}
            ),
        ]
    )
    ...
    for wins in (True, False):
        ...
        assert apply(base, combo).compose(right_prime) == apply(base, edit).compose(left_prime), wins


def test_transform_discovers_a_cut_riding_another_paste_change(cell_handler):
    base = MoveDelta().insert("AB").insert(cell("Hello")).insert("CD")
    outer = MoveDelta().retain(2).cut("c", 1).retain(2).paste("c", 0, 1)
    inner = MoveDelta(
        [
    ...
    combo = outer.compose(inner)
    for priority in (True, False):
        assert combo.transform(MoveDelta(), priority).ops == []
        edit = MoveDelta([{"retain": 2}, cell_patch({"retain": 1, "attributes": {"bold": True}})])
        edit_prime = combo.transform(edit, priority)
        combo_prime = edit.transform(combo, not priority)
        assert apply(base, combo).compose(edit_prime) == apply(base, edit).compose(combo_prime)

    into_change = MoveDelta(
        [
            {"cut": {"ref": "m", "length": 2}},
            {"retain": 2},
            cell_patch({"retain": 5}, {"paste": {"ref": "m", "start": 0, "length": 2}}),
    ...
    combo = outer.compose(into_change)
    edit = MoveDelta().retain(1, bold=True)
    for priority in (True, False):
        edit_prime = combo.transform(edit, priority)
        combo_prime = edit.transform(combo, not priority)
        assert apply(base, combo).compose(edit_prime) == apply(base, edit).compose(combo_prime)
...
def test_transform_follows_destination_into_another_embed(cell_handler):
    ...
    mover = MoveDelta(
        [
            {"cut": {"ref": "x", "length": 2}},
            cell_patch({"retain": 1}, {"paste": {"ref": "x", "start": 0, "length": 2}}),
        ]
    )
    other = MoveDelta(
        [
            {"retain": 2, "attributes": {"bold": True}},
            {"cut": {"ref": "f", "length": 1}},
            cell_patch({"retain": 1}, {"paste": {"ref": "f", "start": 0, "length": 1}}),
        ]
    )
    for wins in (True, False):
        right_prime = mover.transform(other, wins)
        left_prime = other.transform(mover, not wins)
        assert apply(base, mover).compose(right_prime) == apply(base, other).compose(left_prime), wins


def test_coordinate_maps_through_a_paste_change(cell_handler):
    move = MoveDelta().retain(2).cut("e", 1).retain(1).paste("e", 0, 1, change={"cell": {"ops": [{"delete": 2}]}})
...
def test_check_reports_malformed_specs():
    ...
    with pytest.raises(ValueError):
        check(MoveDelta([{"paste": {"ref": "r", "length": 3}}, {"cut": {"ref": "r", "length": 3}}]))
    # malformed specs never crash length-consuming paths either
    assert len(MoveDelta([{"paste": {"ref": "r", "start": 0}}])) == 1


def test_move_discovery_ignores_opaque_embed_data():
    opaque = MoveDelta().insert(
        {
            "opaque": {
                "ops": [
    ...
    assert not has_moves(opaque)
    assert check(opaque) is opaque
    assert opaque.lower(MoveDelta()) == opaque
    assert MoveDelta().transform(opaque) == opaque


def test_move_discovery_visits_only_handler_declared_streams(cell_handler):
    nested = MoveDelta().insert(
        {
            "cell": {
                "ops": [
                ...
                "metadata": {"ops": [{"cut": {"ref": "opaque", "length": 1}}]},
            }
        }
    )
    assert has_moves(nested)
    assert check(nested) is nested


def test_alpha_renaming_does_not_touch_opaque_embed_data():
    first = MoveDelta().cut("same", 1).paste("same", 0, 1)
    opaque = {"opaque": {"ops": [{"paste": {"ref": "same", "start": 0, "length": 1}}]}}
    composed = first.compose(MoveDelta().retain(1).insert(opaque))
    assert composed.ops[-1]["insert"] == opaque
...
def test_fuzz_closure_compose_outputs_transform(cell_handler):
    ...
    for _ in range(120):
        ...
        for wins in (True, False):
            right_prime = composed.transform(edit, wins)
            left_prime = edit.transform(composed, not wins)
            assert after2.compose(right_prime) == apply(base, edit).compose(left_prime), (
                base.ops,
                composed.ops,
                edit.ops,
                wins,
        ...
        # ...compose onwards...
        onwards = (
            random_two_level_edit(rng, after2)
...
def test_reads_race_cell_edits_convergently(cell_handler):
    ...
    edits = (
        ...
        MoveDelta([{"retain": 2}, cell_patch({"retain": 1}, {"delete": 1})]),
        MoveDelta([{"retain": 2}, cell_patch({"retain": 2}, {"retain": 2, "attributes": {"bold": True}})]),
        MoveDelta([{"retain": 2}, cell_patch({"retain": 2}, {"insert": "Z"})]),
        ...
        MoveDelta(
            [
                {"retain": 2},
                cell_patch(
                    {"cut": {"ref": "w", "length": 1}}, {"retain": 2}, {"paste": {"ref": "w", "start": 0, "length": 1}}
                ),
            ]
        ),
...
def test_read_against_gap_delete_converges(cell_handler):
    ...
    composed = MoveDelta(
        [
            {"retain": 1},
            {"paste": {"ref": "trash", "start": 0, "length": 3, "unit": 0, "path": ["ops"]}},
            {"cut": {"ref": "trash", "length": 1}},
        ]
    )
    # the concurrent move's gap deletes the very embed the read addresses
    mover = MoveDelta(
        [{"cut": {"ref": "b", "length": 2}}, {"retain": 1}, {"paste": {"ref": "b", "start": 0, "length": 1}}]
    )
        ...
        .push(
            {
                "insert": {
                    "cell": {
                        "ops": [{"insert": "["}, {"paste": {"ref": "m", "start": 0, "length": 5}}, {"insert": "]"}]
                    }
                }
            }
        )
    )
    assert apply(doc, move) == MoveDelta([{"insert": " world"}, {"insert": {"cell": {"ops": [{"insert": "[Hello]"}]}}}])
...
def random_carried_move(rng, length, ref="c"):
    ...
    for order, (window, carry) in enumerate(zip(windows, carries), 1):
        target = rng.choice([q for q in range(length + 1) if q <= source or q >= source + size])
        events.append((target, order, "carry" if carry else "paste", window, rng.choice(FUZZ_ATTRS)))
    events.sort(key=lambda event: (event[0], rng.random()))
    delta = MoveDelta()
    position = 0
    for target, _, kind, window, attrs in events:
        ...
        if kind == "cut":
            ...
            delta.push(
                carried(
                    ref, window[0], window[1], attrs, wrap=1 if rng.random() < 0.3 else 0, filler=rng.random() < 0.5
                )
            )
    return delta
...
def test_fuzz_insert_carried_compose_laws(cell_handler):
    ...
    for _ in range(300):
        ...
        if len(mid) < 2:
            continue
        second = random_carried_move(rng, len(mid), ref="d")
        assert apply(mid, second) == apply(base, pre.compose(second))
...
def test_fuzz_insert_carried_transform_vs_ordinary(cell_handler):
    ...
    for _ in range(300):
        ...
        for priority in (True, False):
            ...
            assert apply(apply(base, move), edit_prime) == apply(apply(base, edit), move_prime), priority
...
def test_fuzz_insert_carried_transform_vs_move(cell_handler):
    ...
    for _ in range(300):
        ...
        for wins in (True, False):
            ...
            assert apply(apply(base, left), right_prime) == apply(apply(base, right), left_prime), wins
...
def test_fuzz_insert_carried_closure(cell_handler):
    ...
    for _ in range(200):
        ...
        assert not has_moves(combo.lower(base)) and apply(base, combo.lower(base)) == after
        edit = random_ordinary(rng, len(base))
        for priority in (True, False):
            combo_prime = edit.transform(combo, not priority)
            edit_prime = combo.transform(edit, priority)
            assert apply(after, edit_prime) == apply(apply(base, edit), combo_prime), priority
...
def test_fuzz_reads_race_concurrent_movers(cell_handler):
    ...
    for _ in range(400):
        ...
        for wins in (True, False):
            right_prime = composed.transform(mover, not wins)
            left_prime = mover.transform(composed, wins)
            assert apply(base, composed).compose(right_prime) == apply(base, mover).compose(left_prime), (
                mover.ops,
                wins,
            )
...
def test_fuzz_embed_diff_law(cell_handler):
    ...
    for _ in range(200):
        a = MoveDelta().insert(cell("".join(rng.choice(LETTERS) for _ in range(rng.randint(1, 6)))))
        b = MoveDelta().insert(cell("".join(rng.choice(LETTERS) for _ in range(rng.randint(1, 6)))))
...
def test_compose_splits_retain_carried_windows(cell_handler):
    ...
    move = MoveDelta(
        [
            {"cut": {"ref": "m", "length": 5}},
            {"retain": 3},
            {"retain": {"cell": {"ops": [{"retain": 1}, {"paste": {"ref": "m", "start": 0, "length": 5}}]}}},
        ]
    )
    combo = pre.compose(move)
    assert apply(apply(doc, pre), move) == apply(doc, combo)
...
def test_compose_expands_a_nested_child_move_once(cell_handler):
    base = MoveDelta().insert({"cell": {"ops": [{"insert": cell("AB")}, {"insert": "x"}]}})
    axis_move = [
...
def test_fuzz_retain_carried_compose(cell_handler):
    ...
    for _ in range(200):
        ...
        pre = random_ordinary(rng, len(base))
        mid = apply(base, pre)
        if len(mid) < 3 or not any(isinstance(o.get("insert"), dict) for o in mid.ops):
            continue
        ...
        if isinstance(insert, dict) and "cell" in insert:
            sites.append(
                (
                    position,
                    sum(
                        op.str_length(o["insert"]) if isinstance(o.get("insert"), str) else 1
...
def random_cell_to_cell_move(rng, document, ref="r"):
    ...
    cut = cell_patch(*([{"retain": offset}] if offset else []), {"cut": {"ref": ref, "length": size}})
    ...
    (first_pos, first), (second_pos, second) = sorted([(src_pos, cut), (dst_pos, paste)], key=lambda site: site[0])
    return MoveDelta().retain(first_pos).push(first).retain(second_pos - first_pos - 1).push(second)
...
def random_two_cell_edit(rng, document):
    ...
    for operation in document.ops:
        ...
        if isinstance(insert, dict):
            inner = sum(
                op.str_length(o["insert"]) if isinstance(o.get("insert"), str) else 1 for o in insert["cell"]["ops"]
            )
...
def test_cell_to_cell_transform_follows_source_edits(cell_handler):
    ...
    for priority in (True, False):
        ...
        assert left.ops == [
            {"insert": {"cell": {"ops": []}}},
            {"insert": {"cell": {"ops": [{"insert": "two"}, {"insert": "one", "attributes": {"bold": True}}]}}},
        ]
...
def test_compose_splits_cell_to_cell_paste(cell_handler):
    ...
    later = MoveDelta([{"retain": 1}, cell_patch({"retain": 4}, {"insert": "!"})])
    composed = move.compose(later)
    assert apply(base, composed) == apply(base, move).compose(later)
...
def test_fuzz_cell_to_cell_moves(cell_handler):
    ...
    for _ in range(200):
        ...
        for priority in (True, False):
            ...
            assert apply(apply(base, move), edit_prime) == apply(apply(base, concurrent), move_prime), priority
...
def test_fuzz_cell_to_cell_vs_concurrent_moves(cell_handler):
    ...
    for _ in range(200):
        ...
        right = (
            random_cell_to_cell_move(rng, base, ref="z") if rng.random() < 0.6 else random_move(rng, len(base), ref="z")
        )
        for wins in (True, False):
            right_prime = left.transform(right, wins)
            left_prime = right.transform(left, not wins)
...
def test_compose_expands_a_window_riding_a_paste_change(cell_handler):
    ...
    B = MoveDelta(
        [
            cell_patch({"retain": 1}, {"paste": {"ref": "b", "start": 0, "length": 1}}),
            {"cut": {"ref": "b", "length": 1}},
        ]
    )
    AB = A.compose(B)
...
def test_compose_never_expands_handler_composed_windows_twice(cell_handler):
    ...
    B = MoveDelta(
        [
            {"retain": 2},
            cell_patch({"retain": 1}, {"paste": {"ref": "b", "start": 0, "length": 3}}),
            {"cut": {"ref": "b", "length": 3}},
        ]
    )
    AB = A.compose(B)
...
def test_compose_expands_a_passed_through_sibling_window():
    ...
    # passed-through lane (regression from a real 104-revision history
    # chain; fixture by codex)
    class LanesHandler:
        @staticmethod
        def stream_paths(value):
            return [
                ...
                if isinstance(value.get(lane), dict) and isinstance(value[lane].get("ops"), list)
            ]

        @staticmethod
        def apply(a, b, context):
            return LanesHandler.compose(a, b, context)

        @staticmethod
        def compose(a, b, context):
            result = {}
            for lane in ("left", "right"):
                if lane in a and lane in b:
                    result[lane] = {
                        "ops": MoveDelta(a[lane]["ops"]).compose(MoveDelta(b[lane]["ops"]), _context=context).ops
                    }
                elif lane in b:
                    result[lane] = b[lane]
                elif lane in a:
    ...
    Delta.register_embed("lanes", LanesHandler)
    try:
        base = MoveDelta().insert(
            {
                "lanes": {
                    "left": {"ops": [{"insert": "A"}]},
                    "right": {"ops": [{"insert": "B"}]},
        ...
        Delta.unregister_embed("lanes")


# ---------------------------------------------------------------------------
# moves with the production-shaped table handler
# ---------------------------------------------------------------------------
# TableHandler (tests/embed_handlers.py) behaves nothing like the
# cooperative CellHandler: its patches carry their own base snapshot,
# compose applies-and-rediffs, and cell content is a raw list — opaque
# to the move core.  These tests pin the algebra against that profile.

from tests.embed_handlers import TableHandler, registered_embed_handler  # noqa: E402


def table(cells):
    """A one-row table embed: cells = {'c1': 'text', ...}."""
    return {
        "table-embed": {
            "rows": [{"insert": {"id": "r1"}}],
            "columns": [{"insert": {"id": cid}} for cid in cells],
            "cells": {f"r1:{cid}": {"content": [{"insert": text}]} for cid, text in cells.items()},
        }
    }


def table_patch(data, cell, text):
    """A self-contained table patch rewriting one cell's content."""
    target = copy.deepcopy(data)
    target["cells"][cell] = {"content": [{"insert": text}]}
    return TableHandler.diff(data, target, None)


def test_table_patch_follows_a_root_move():
    with registered_embed_handler("table-embed"):
        embed = table({"c1": "A", "c2": "B"})
        base = MoveDelta().insert("xy").insert(embed).insert("z")
        move = MoveDelta().retain(2).cut("m", 1).retain(1).paste("m", 0, 1)
        edit = MoveDelta(
            [{"retain": 2}, {"retain": {"table-embed": table_patch(embed["table-embed"], "r1:c1", "EDITED")}}]
        )
        for priority in (True, False):
            move_prime = edit.transform(move, not priority)
            edit_prime = move.transform(edit, priority)
            ...
            right = apply(apply(base, edit), move_prime)
            assert left == right, priority
            cells = left.ops[-1]["insert"]["table-embed"]["cells"]
            assert cells["r1:c1"]["content"] == [{"insert": "EDITED"}]


def test_table_patch_rides_a_move_and_inverts():
    with registered_embed_handler("table-embed"):
        embed = table({"c1": "A"})
        base = MoveDelta().insert("xy").insert(embed).insert("z")
        move = MoveDelta().retain(2).cut("m", 1).retain(1).paste("m", 0, 1)
        later = MoveDelta(
            [{"retain": 3}, {"retain": {"table-embed": table_patch(embed["table-embed"], "r1:c1", "NEW")}}]
        )
        combo = move.compose(later)
        check(combo)
        spec = next(o["paste"] for o in combo.ops if "paste" in o)
        assert "table-embed" in spec["change"]  # the patch rides the paste
        after = apply(base, combo)
        assert after == apply(apply(base, move), later)
        inverse = combo.invert(base)
        check(inverse)
        assert after.compose(inverse) == base


def test_fuzz_table_embed_moves():
    rng = random.Random(95)
    with registered_embed_handler("table-embed"):
        for _ in range(150):
            texts = {f"c{i}": "".join(rng.choice(LETTERS) for _ in range(2)) for i in range(1, rng.randint(2, 4))}
            embed = table(texts)
            prefix = "".join(rng.choice(LETTERS) for _ in range(3))
            base = MoveDelta().insert(prefix).insert(embed).insert("".join(rng.choice(LETTERS) for _ in range(2)))
            unit = op.str_length(prefix)
            move = random_move(rng, len(base), ref="m")
            after = apply(base, move)
            assert after.compose(move.invert(base)) == base
            cell = f"r1:{rng.choice(list(texts))}"
            edit = MoveDelta(
                [{"retain": unit}, {"retain": {"table-embed": table_patch(embed["table-embed"], cell, "X")}}]
            )
            for priority in (True, False):
                move_prime = edit.transform(move, not priority)
                edit_prime = move.transform(edit, priority)
...
def test_fuzz_history_chains(cell_handler):
    """Real-usage shape: long edit histories folded through compose,
    validity and the apply law checked at every prefix, then the whole
    chain undone by composing the inverses newest-first."""
    ...
    for _ in range(8):
        ...
        for _step in range(30):
            ...
            try:
                if roll < 0.35:
                    delta = random_ordinary(rng, len(document))
                elif roll < 0.55:
                    delta = random_move(rng, len(document), ref="r")
                elif roll < 0.7:
                    delta = random_cross_move(rng, document, ref="x")
                elif roll < 0.85:
                    delta = random_cell_to_cell_move(rng, document, ref="c")
                else:
                    delta = random_carried_move(rng, len(document), ref="w")
            except ValueError, IndexError:
                delta = random_ordinary(rng, len(document))
            inverses.append(delta.invert(document))
            document = apply(document, delta)
            combo = delta if combo is None else combo.compose(delta)
            check(combo)
            assert apply(base, combo) == document  # law at every prefix
        undo = None
        for inverse in reversed(inverses):
            undo = inverse if undo is None else undo.compose(inverse)
            check(undo)
        assert apply(document, undo) == base
